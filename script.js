...
function renderSchedule() {
    ...
    list.innerHTML = NS.schedule.map((s, i) => {
        ...
        const dur = s.item.duration_min ? Math.round(parseFloat(s.item.duration_min)) + 'min' : '';
        return `<div class="schedule-item ${isNow ? 'now-playing' : ''}" data-idx="${i}">
            <span class="sched-time">${s.time}</span>
            <span class="sched-title">${s.item.title || 'TBD'}</span>
            <span class="sched-type">${(s.item.content_type || '').replace('_',' ')}</span>
